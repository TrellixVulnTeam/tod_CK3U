...
class WAConnection extends _1_Validation_1.WAConnection {
    ...
    async connect() {
        ...
        const result = { user: this.user, newConnection, ...(updates || {}) };
    ...
    receiveChatsAndContacts(waitOnlyForLast) {
        const rejectableWaitForEvent = (event) => {
            let rejectTask = (_) => { };
        ...
        if (!waitOnlyForLast)
            events.push('CB:action,add:before', 'CB:action,add:unread');
        const cancellations = [];
        const wait = Promise.all(events.map(ev => {
        ...
        if (message[0] === '!') {
            ...
            try {
                ...
                if (this.state === 'open')
                    ...
                    this.emit('ws-close', new Error(Constants_1.DisconnectReason.badSession));
            }
