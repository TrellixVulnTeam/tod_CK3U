...
class WAConnection extends _3_Connect_1.WAConnection {
    constructor() {
        ...
        this.chatUpdateTime = (chat, stamp) => this.chats.updateKey(chat, c => c.t = stamp);
        // chats received
            ...
            chats.all().forEach(chat => {
                ...
                if (!oldChat) {
                    ...
                    if (oldChat.t !== chat.t || oldChat.modify_tag !== chat.modify_tag) {
                        const changes = Utils_1.shallowChanges(oldChat, chat);
        ...
        this.on('CB:Chat,cmd:action', json => {
            ...
            if (data) {
                const emitGroupParticipantsUpdate = (action) => this.emit('group-participants-update', { participants: data[2].participants.map(Utils_1.whatsappID), actor: data[1], jid: json[1].id, action });
                switch (data[0]) {
                    case "promote":
        ...
        this.on('CB:action,add:relay,received', json => {
            ...
            if (json.type === 'error') {
                const update = {
                    from: this.user.jid,
                    to: Utils_1.whatsappID(json.jid),
        ...
        const func = json => {
            ...
            if (json.cmd === 'ack')
                ids = [json.id];
            const update = {
                from: Utils_1.whatsappID(json.from),
                to: Utils_1.whatsappID(json.to),
                participant: Utils_1.whatsappID(json.participant),
    ...
    chatAddMessage(message, chat) {
        ...
        if (!message.key.fromMe && message.message) {
            ...
            if (contact.lastKnownPresence === Constants_1.Presence.composing) { // update presence
                const update = this.applyingPresenceUpdate({ id: chat.jid, participant: message.participant || chat.jid, type: Constants_1.Presence.available });
        ...
        if (protocolMessage) {
            ...
            if (message.message) {
                this.chatUpdateTime(chat, +Utils_1.toNumber(message.messageTimestamp));
                chatUpdate.t = chat.t;
            }
            ...
            if (message.messageStubType) {
                ...
                let participants;
                const emitParticipantsUpdate = (action) => this.emit('group-participants-update', { jid, actor, participants, action });
                const emitGroupUpdate = (update) => this.emit('group-update', { jid, actor, ...update });
