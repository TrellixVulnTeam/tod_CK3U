...
class WAConnection extends _4_Events_1.WAConnection {
    constructor() {
        ...
        this.isOnWhatsApp = async (str) => {
            ...
            if (status === 200)
                return { exists: true, jid: Utils_1.whatsappID(jid) };
        };
        /**
        ...
        this.updatePresence = (jid, type) => this.query({
            ...
            binaryTags: [Constants_1.WAMetric.group, Constants_1.WAFlag.acknowledge],
            expect200: true
        });
    ...
    async getContacts() {
        ...
        const response = await this.query({ json, binaryTags: [6, Constants_1.WAFlag.ignore], expect200: true }); // this has to be an encrypted query
        return response;
    }
    /** Get the stories of your contacts */
    async getStories() {
        const json = ['query', { epoch: this.msgCount.toString(), type: 'status' }, null];
        const response = await this.query({ json, binaryTags: [30, Constants_1.WAFlag.ignore], expect200: true });
    ...
    async deleteChat(jid) {
        const response = await this.setQuery([['chat', { type: 'delete', jid: jid }, null]], [12, Constants_1.WAFlag.ignore]);
     ...
     */
    async loadChats(count, before, options = {}) {
        var _a;
        ...
        if (typeof options.loadProfilePicture !== 'undefined')
            loadPP = options.loadProfilePicture;
        if (loadPP) {
            await Promise.all(chats.map(async (chat) => (typeof chat.imgUrl === 'undefined' && await this.setProfilePicture(chat))));
        }
